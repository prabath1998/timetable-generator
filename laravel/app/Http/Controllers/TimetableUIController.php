...
class TimetableUIController extends Controller
{
    ...
    public function generate(Request $request)
    {
        ...
        $assignments = DB::table('group_teacher')
            ...
            ->select('group_teacher.group_id','group_teacher.teacher_id','group_teacher.subject_id',
    ...
    public function show($id)
    {
        ...
        $entries = DB::table('timetable_entries as e')
            ...
            ->select('e.*','g.name as group_name','t.name as teacher_name','s.name as subject_name',
        ...
        // build fast lookup maps for grid rendering
        // map[group_id][day][slot] = {teacher_name, subject_name}
        $byGroup = [];
        // map[teacher_id][day][slot] = {group_name, subject_name}
        $byTeacher = [];
        foreach ($entries as $row) {
            $byGroup[$row->group_id][$row->day_of_week][$row->slot_index] = [
                'subject' => $row->subject_name,
                'teacher' => $row->teacher_name
            ];
            $byTeacher[$row->teacher_id][$row->day_of_week][$row->slot_index] = [
                'subject' => $row->subject_name,
                'group'   => $row->group_name
            ];
        }

        // Meta for grid headers
        $days = $timeslots->pluck('day_of_week')->unique()->values()->all();
        $slots = $timeslots->pluck('slot_index')->unique()->values()->all();



        return view('timetable.show', compact(
            'id','req','days','slots','groups','teachers','byGroup','byTeacher'
        ));
    }
    ...
    public function move(Request $request, $id)
{
    ...
    if (!$entry) return response()->json(['ok'=>false,'message'=>'Entry not found'], 404);
    ...
    if ($swapWith) {
        ...
        $b = DB::table('timetable_entries')
            ->where('id', $swapWith)
            ->where('timetable_request_id', $id)->first();
        if (!$b) return response()->json(['ok'=>false,'message'=>'Swap target not found'], 404);
